...
(defn ^:private format-str [at action message-type header-details body]
...
(defn ^:private format-notification [action notif at]
  (format-str at action "notification" (format-notification-signature notif)
              (format-params notif)))

(defn ^:private format-request [action req at]
  (format-str at action "request" (format-request-signature req)
              (format-params req)))

(defn ^:private format-response [action req {:keys [error] :as resp} started finished]
  (format-str finished action "response"
              (format
