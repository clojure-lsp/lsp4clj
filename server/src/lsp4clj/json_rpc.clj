...
(defn ^:private read-n-chars [^java.io.Reader reader content-length _charset-s]
  (let [buffer (char-array content-length)]
    (loop [total-read 0]
      (when (< total-read content-length)
        (let [new-read (.read reader buffer total-read (- content-length total-read))]
          (when (< new-read 0)
            ;; TODO: return nil instead?
            (throw (java.io.EOFException.)))
          (recur (+ total-read new-read)))))
    ;; TODO: using charset causes reflection warning
    (String. buffer #_charset-s)))
                    ...
                    (when-let [content-type (get headers "Content-Type")]
                      (when-let [[_ charset] (re-find #"; charset=(.*)$" content-type)]
                        (when (not= "utf8" charset)
                          charset)))
                    "utf-8")
        content (read-n-chars reader content-length charset-s)]
  ...
  (let [msgs (async/chan 1)]
    (async/go-loop [headers {}]
      (if-let [line (async/<! (read-line-async reader))]
        (if (string/blank? line) ;; a blank line after the headers indicates start of message
          (do
            (async/>! msgs (read-message reader headers))
            (recur {}))
